...
- [ ] **Bed collapse toggles the wrong bed.** With exactly two beds, clicking one bed's header collapses or expands the *other* one. Root cause (`GardenPlanner.tsx`, the `BedGrid` call site): a single nullable `expandedBedId` encodes an accordion — one bed open at a time — but its `null` default is overloaded to mean "all open" when there are ≤ 2 beds, and the two models collide. Start at `null`: both beds show expanded. Click bed A: state becomes `"A"`, so A stays open and **B collapses**. Click A again: back to `null`, both open. Clicking A can therefore never collapse A; it only ever toggles B. Needs per-bed collapse state (e.g. a set of collapsed ids) so beds toggle independently. Note the whole header row is one click target, so badges and labels toggle it too — only the rename input stops propagation.
