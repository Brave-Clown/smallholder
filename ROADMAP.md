...
- [ ] **Bed expand/collapse: make the two viewing styles a real choice.** Today they are tangled together and the result is a bug — with exactly two beds, clicking one bed's header collapses or expands the *other* one.

  *Root cause* (`GardenPlanner.tsx`, the `BedGrid` call site): a single nullable `expandedBedId` encodes an accordion — one bed open at a time — but its `null` default is overloaded to mean "all open" when there are ≤ 2 beds, so the two models collide. Start at `null`: both beds show expanded. Click bed A: state becomes `"A"`, so A stays open and **B collapses**. Click A again: back to `null`, both open. Clicking A can therefore never collapse A; it only ever toggles B.

  *Wanted*: both styles, deliberately, behind a toggle — **one bed at a time** (accordion, which is the behaviour that was hiding in here and is worth keeping) and **as many beds open as you like**. One nullable field cannot express both; needs a mode flag plus a set of open (or collapsed) bed ids, where accordion mode simply closes the others on open.

  *Notes for whoever picks this up*: the mode is a view preference, so it belongs alongside `gridCellSizeCm` and `theme` in `settingsSlice`, not as component state that resets on reload. Decide the default — the current `≤ 2 beds → all open` heuristic was groping toward "all open is friendlier when there are few beds", which may be worth keeping as the initial value rather than a permanent rule. Per the product principle the toggle must be ignorable: whichever default ships should be the one a user who never finds the control is happy with. Needs an i18n key. Also note the whole header row is a single click target, so badges and labels toggle it too — only the rename input stops propagation.
