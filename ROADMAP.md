...
- [ ] Downgrade companion conflicts everywhere to soft warnings with per-placement override. Includes fixing the cell tooltip, which shows the raw key — `⚠ validation.antagonistDirect` — because `cellWarnings` (`GardenPlanner.tsx`) stores `issues[0].messageKey` and `DroppableCell` interpolates it into `title` untranslated, dropping `messageParams` entirely. The click path already does this properly via `resolveParams`, which resolves plant ids to names; the tooltip needs the same treatment. Worth doing here rather than separately, since the severity rework rewrites this code anyway
